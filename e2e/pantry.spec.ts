...
test.describe('Pantry (auth disabled)', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('/api/pantry', (route) => {
      if (route.request().method() === 'GET') {
        route.fulfill({ json: mockPantryItems });
      } else {
        route.continue();
      }
    });
  ...
  test('pantry page shows items and the scan action', async ({ page }) => {
    await page.goto('/pantry');
    await expect(page.getByText('Milk')).toBeVisible();
    // The scan link is behind the "Add to pantry" FAB, not visible until opened.
  ...
  test('scan button links to scan page', async ({ page }) => {
    ...
    await expect(page).toHaveURL('/pantry/scan');
    await expect(page.getByText('Scan barcode')).toBeVisible();
  });

  test('clicking a pantry item opens its detail page', async ({ page }) => {
    await page.goto('/pantry');
    await page.getByText('Milk').click();
    await expect(page).toHaveURL(/\/pantry\/item-1/);
  });
});
...
test.describe('Binning the last of an item (auth disabled)', () => {
  ...
  test('binning it all removes the item from the pantry list rather than leaving a 0-remaining row', async ({ page }) => {
    ...
    // real backend's archive-and-delete behavior (see PantryService.saveOrArchive).
    let items = [...mockPantryItems];
    ...
    await expect(page.getByText('No items in pantry')).toBeVisible();
