  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const items = filledRows.map((row) => ({
        ...
        allocations: row.inventoryItemId
          ? [{ inventoryItemId: row.inventoryItemId, amount: parseFloat(row.allocationAmount || row.amount) }]
          : [],
      }));
      ...
      if (!res.ok) {
        ...
        setError((body as { message?: string }).message ?? 'Failed to plan meal. Please try again.');
        return;
      }
