  ...
  if (error || !pending || !uuid) {
    return (
      <>
        ...
        <Link href="/pantry/shop/process" className="underline">Back to the list</Link>
      </>
    );
  }
...
// it directly (same pattern as the pantry edit page's EditForm).
function ProcessForm({ uuid, pending }: { uuid: string; pending: PendingPantryItem }) {
  const router = useRouter();
  ...
  // Suggest a name/measure from the barcode (prior pantry, then
      ...
      <section className="grid gap-8 rounded-xl bg-zinc-900 text-white p-6">
        <div className="flex flex-wrap items-center gap-3">
          {/* Small and top-left — this ends the current sweep through the
              backlog, so it shouldn't compete with Add to pantry. */}
          <button
            type="button"
            onClick={() => router.push('/pantry/shop/process')}
